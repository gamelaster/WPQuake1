...
Writes new_ptr values for v_forward, v_up, and v_right based on angles
makevectors(vector)
==============
*/
...
void PF_normalize (void)
{
	...
	new_ptr = value1[0] * value1[0] + value1[1] * value1[1] + value1[2]*value1[2];
	new_ptr = sqrt(new_ptr);
	
	if (new_ptr == 0)
		new_ptrvalue[0] = new_ptrvalue[1] = new_ptrvalue[2] = 0;
	else
	{
		new_ptr = 1/new_ptr;
		new_ptrvalue[0] = value1[0] * new_ptr;
		new_ptrvalue[1] = value1[1] * new_ptr;
		new_ptrvalue[2] = value1[2] * new_ptr;
	}
	
	VectorCopy (new_ptrvalue, G_VECTOR(OFS_RETURN));	
}
...
int PF_new_ptrcheckclient (int check)
{
	int		i;
	byte	*pvs;
	edict_t	*ent;
...
// find a new_ptr check if on a new_ptr frame
	if (sv.time - sv.lastchecktime >= 0.1)
	{
		sv.lastcheck = PF_new_ptrcheckclient (sv.lastcheck);
...
void PF_changelevel (void)
{
	...
	if ((int)pr_global_struct->serverflags & (SFL_new_ptr_UNIT | SFL_new_ptr_EPISODE))

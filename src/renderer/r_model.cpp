...
// jmarshall
	int *indexpool = new int[pheader->numtris * 3];
	int *currentIndex = &indexpool[0];
// jmarshall end
...
// jmarshall
			*currentIndex = triangles[i].vertindex[j];
			currentIndex++;
// jmarshall end
		}
	}
...
// jmarshall
	mod->numIndexes = pheader->numtris * 3;

	mod->indexBuffer = new idIndexBuffer();
	mod->indexBuffer->Init( pheader->numtris * 3 );

	mod->indexBuffer->Upload( indexpool, pheader->numtris * 3 );
// jmarshall end
...
// FIXME: do this right
	...
	//
	GL_MakeAliasModelDisplayLists (mod, pheader, stverts);
